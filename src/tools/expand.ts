...
export class ExpandAction implements Action {
    ...
    constructor(public readonly expandElementsIDs: string[] = [],
...
export class NodeExpandTool extends DiagramTool {
    ...
    constructor(
        ...
        @inject(ToolTYPES.IFeedbackActionDispatcher) protected feedbackDispatcher: IFeedbackActionDispatcher,
        ) {
            super() 
        }
...
export class NodeExpandToolMouseListener extends DragAwareMouseListener {
    ...
    wheel(target: SModelElement, event: MouseEvent): Action[] {
        ...
        if (event.button === 0) {
            ...
            if (selectableTarget !== undefined || target instanceof SModelRoot) {
                ...
                if (!isCtrlOrCmd(event)) {
                    exiting = toArray(target.root.index.all()
        ...
        return [new SelectAction(entering.map(e => e.id), exiting.map(e => e.id))];
    }
