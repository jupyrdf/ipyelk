...
from ipyelk.elements import Node, Label, LabelProperties, ElementMetadata, LabelProperties, layout_options as opts
from typing import List, Any, Iterator, Tuple, Dict, Hashable
...
class PandasLoader(ElementLoader):
    ...
    def load(self, node_data:pd.DataFrame, edges:pd.DataFrame=None, by:List[str]=None, attrs:List[str]=None):
        ...
        if depth:
            ...
            if by_col_nulls:
                warnings.warn("Following `by` columns contain null values. {}".format(by_col_nulls))
