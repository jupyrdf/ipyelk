...
class Toolbar(W.HBox, StyledWidget):
    ...
    on_close = T.Any(default_value=None)  # holds a callable function to execute when close button is pressed
    ...
    def _update_close_callback(self, change:T.Bunch=None):
        """Toggle visiblity of the close button depending on if the `on_close` trait is callable"""
